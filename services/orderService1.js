...
module.exports = {
    ...
    getData: async (req, res) => {
        try {
            ...
            res.send(resultMessage.success({
                orderNum, orderPrice, todayNum, todayMoney,
            }));
        } catch (error) {
            console.log(error);
            return res.send(resultMessage.error([]));
    ...
    getMoneyByShopid: async (req, res) => {
        ...
        try {
            sequelize.query(str, { replacements: [shopid], type: sequelize.QueryTypes.SELECT }).then((projects) => {
                res.send(resultMessage.success(projects));
            });
        } catch (error) {
            console.log(error);
    ...
    getOrderByStatusAndPosition: async (req, res) => {
        ...
        try {
            const list = await orderModel.findAll(params);
            const result = [];
            list.map((item) => {
                const obj = {
                    id: item.id,
